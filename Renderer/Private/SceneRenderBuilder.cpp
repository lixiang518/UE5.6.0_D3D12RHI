...
public:
	...
	TArray<FSceneRenderer*, FConcurrentLinearArrayAllocator> CreateSceneRenderers(
		TConstArrayView<FSceneViewFamily*> ViewFamilies,
		TConstArrayView<FRuntimeSceneView*> RuntimeSceneView,
		FHitProxyConsumer* HitProxyConsumer
		) const
	{

		if (!RuntimeSceneView.Num())
		{
			return {};
		}

		const FSceneInterface* SceneInterface = RuntimeSceneView[0]->Scene;
		check(SceneInterface);
		check(SceneInterface->GetRenderScene() == Scene);

		TArray<FSceneRenderer*, FConcurrentLinearArrayAllocator> OutRenderers;
		OutRenderers.Reserve(RuntimeSceneView.Num());
		...
		// for (FRuntimeSceneView* ViewFamily : RuntimeSceneView)
		{
			...
			OutRenderers.Add(new RuntimeRender(ViewFamilies[0], RuntimeSceneView[0], HitProxyConsumer));
...
TArray<FSceneRenderer*, FConcurrentLinearArrayAllocator> FSceneRenderBuilder::CreateSceneRenderers(
	TConstArrayView<FSceneViewFamily*> ViewFamilies,
	TConstArrayView<FRuntimeSceneView*> RuntimeSceneViews,
	FHitProxyConsumer* HitProxyConsumer)
{
	LazyInit();
	TArray<FSceneRenderer*, FConcurrentLinearArrayAllocator> Renderers = Processor->CreateSceneRenderers(ViewFamilies, RuntimeSceneViews, HitProxyConsumer);
...
TArray<FSceneRenderer*, FConcurrentLinearArrayAllocator> FSceneRenderBuilder::CreateLinkedRuntimeSceneRenderers(
	TConstArrayView<FSceneViewFamily*> ViewFamilies,
	TConstArrayView<FRuntimeSceneView*> RuntimeSceneViews,
	FHitProxyConsumer* HitProxyConsumer)
{
	return CreateSceneRenderers(ViewFamilies, RuntimeSceneViews, HitProxyConsumer);
}
