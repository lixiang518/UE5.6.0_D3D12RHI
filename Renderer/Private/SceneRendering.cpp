...
void FRendererModule::BeginRenderingViewFamily(FCanvas* Canvas, FSceneViewFamily* ViewFamily, FRuntimeSceneView* RuntimeSceneView)
{
	BeginRenderingRuntimeViewScene(Canvas,MakeArrayView({ ViewFamily }), MakeArrayView({ RuntimeSceneView }));
}

void FRendererModule::BeginRenderingRuntimeViewScene(FCanvas* Canvas,TConstArrayView<FSceneViewFamily*> ViewFamilies, TConstArrayView<FRuntimeSceneView*> RuntimeSceneView)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(BeginRenderingViewFamily);
	check(Canvas);
	for (FRuntimeSceneView* ViewFamily : RuntimeSceneView)
	{
		check(ViewFamily);
		check(ViewFamily->Scene == RuntimeSceneView[0]->Scene);
	}

	UWorld* World = nullptr;

	FScene* const Scene = RuntimeSceneView[0]->Scene ? RuntimeSceneView[0]->Scene->GetRenderScene() : nullptr;
	if (Scene)
	{
	...
	if (Scene)
	{
		...
		TArray<FSceneRenderer*, FConcurrentLinearArrayAllocator> SceneRenderers = SceneRenderBuilder.CreateLinkedRuntimeSceneRenderers(ViewFamilies, RuntimeSceneView, Canvas->GetHitProxyConsumer());
		...
		for (FSceneRenderer* SceneRenderer : SceneRenderers)
		{
			...
			SceneRenderBuilder.AddRenderer(SceneRenderer,  TEXT("ViewFamilies"),
